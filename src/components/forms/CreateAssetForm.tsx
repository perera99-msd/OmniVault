    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger
        render={
          <button
            type="button"
            className="px-5 py-3 rounded-2xl bg-emerald-500 hover:bg-emerald-600 text-white font-black text-sm shadow-lg shadow-emerald-500/20 transition-all flex items-center gap-2"
          />
        }
      >
      ...
      <DialogContent className="sm:max-w-xl max-h-[90vh] overflow-y-auto custom-scrollbar bg-white dark:bg-[#121214] border border-zinc-200 dark:border-white/10 p-6 sm:p-8 rounded-[2rem] shadow-2xl">
        <DialogHeader>
          <DialogTitle className="text-2xl font-black tracking-tight text-zinc-900 dark:text-white flex items-center gap-2.5">
            <Sparkles className="w-6 h-6 text-emerald-500" /> Log New Asset
          </DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="space-y-5 mt-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
              <label className="block text-xs font-bold uppercase tracking-wider text-zinc-500 mb-1.5">
                Acquired / Bought Value ({baseCurrency}) *
              </label>
              <input
                type="number"
                step="any"
